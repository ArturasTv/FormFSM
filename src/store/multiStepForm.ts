...
const useBearStore = create<MultiStepFormStore>()((set, get) => ({
  state: "first",
    next: () => {
        const current = transitions.find((transition) => transition.from === get().state && transition.trigger === "next")
        if (current) {
        set({ state: current.to })
        }
    },
    previous: () => {
        const current = transitions.find((transition) => transition.from === get().state && transition.trigger === "previous")
        if (current) {
        set({ state: current.to })
        }
    }
